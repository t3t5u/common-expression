...
public class Divide<T extends Number> extends AbstractBinaryExpression<T, T, T> implements ArithmeticExpression<T> {
	Divide(final Expression<T> leftExpression, final Expression<T> rightExpression) {
	...
	public T evaluate() {
		...
		return (left != null) && (right != null) ? ExtraNumberUtils.toNumber(ExtraNumberUtils.toBigDecimal(left).divide(ExtraNumberUtils.toBigDecimal(right)), getExpressionClass()) : null;
	}
